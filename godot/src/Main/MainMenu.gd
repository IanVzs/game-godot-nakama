...
func _ready():
	...
	while (not has_permissions and OS.get_name() == "Android"):
		var permissions: Array = OS.get_granted_permissions()
		# 貌似是无用代码
		if not permissions.has("android.permission.INTERNET") or not permissions.has("android.permission.ACCESS_NETWORK_STATE"):
			_sign = OS.request_permissions()
